...
# 1. GMST only
# =============
# out/blockmax_1983_2020_dt180_PERSIANN_obs_out.csv
# ac/ ... 

#dLdt <- read.csv("output/event_def_dt180/obs-WBM/PERSIANN/out/blockmax_1983_2020_dt180_PERSIANN_obs_out.csv", sep = ",", comment.char = "#", header = T, col.names = c("year", "dLdt", "NULL"))[,1:2]
dLdt <- read.csv("output/event_def_dt180/obs-WBM/PERSIANN/ac/blockmax_1983_2020_dt180_PERSIANN_obs_ac.csv", sep = ",", comment.char = "#", header = T, col.names = c("year", "dLdt", "NULL"))[,1:2]
...
#write.csv(res, paste( "output/stat-fits-output/lakevic-fit-wbm-obs-out-gmst-", as.character(startyear), "-", as.character(endyear), ".csv", sep="" ), row.names=TRUE)
write.csv(res, paste( "output/stat-fits-output/lakevic-fit-wbm-obs-ac-gmst-", as.character(startyear), "-", as.character(endyear), ".csv", sep="" ), row.names=TRUE)
