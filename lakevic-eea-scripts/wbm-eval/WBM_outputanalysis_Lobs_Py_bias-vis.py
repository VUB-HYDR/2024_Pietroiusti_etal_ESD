...
inDIR_Py = os.path.join(workDIR,r'WBM-git\output\2023\PERSIANN\v01r02\\')  # PERSIANN fillnan
...
# Python version
#=============================================================================
filepath = os.path.join(inDIR_Py, 'WBM_run_PERSIANN_obs_v01r02_1983_2020_mm.csv') # WBM_run_PERSIANN_obs_v04r01_1983_2020_mm.csv 1983-2020
...
#WBM_obs
df = WBM_obs_raw[['date','L_wb', 'P_lake', 'Q_in', 'E_lake', 'Q_out']]
...
#%% 1) Lake level plot : Fig 12 paper 
#=============================================================================
...
ax.plot(LL_obs['water_level'],label="observed", color='C2' ) #(1983-2020)
# plot modelled 
ax.plot(WBM_obs['L_wb'], label="modelled " ) #(observational run)
...
#xlist = pd.to_datetime([1983] + list(range(1988,2020,4)) + [2021], format='%Y')
#xlist = pd.to_datetime([1983] + list(range(1988,2022,4)), format='%Y')
...
#ax.set_xticklabels(ax.get_xticks(), ax.get_xticklabels().strftime("%b-%Y"), rotation = 45, ha="right", rotation_mode="anchor")
...
c1= 'gray'# '0.2'
ax2=ax.twinx()
residual = WBM_obs.loc[str(left):str(right)]['L_wb'] - LL_obs.loc[str(left):str(right)]['water_level']
...
#ax2.set_ylim(-2,14)
ax2.set_ylim(-1,7)
#ax2.set_ylabel(" $\Delta$level (m)", color='gray')
...
fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_zoom20182020.png'),dpi=300)

#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_v9.png'),dpi=300)
#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_v9.pdf'),dpi=300)
...
ax.plot(LL_obs['water_level'],label="observed", color='C0' ) #(1983-2020) #C2
# plot modelled 
ax.plot(WBM_obs['L_wb'], label="modelled ", color='C1' ) #(observational run) #C0
ax.set_ylabel("lake level (m a.s.l.)")
#ax.set_ylim(1133,1137)
ax.grid(False)
...
ax1.plot(df_obs,label="observed", color='C2' ) #(1983-2020)
ax1.plot(df_mod, label="modelled " ) #(observational run)
...
ax3.set_xlabel('')
xlist =  pd.date_range(start='2018-01-01', end='2021-01-01', freq='2MS')
ax3.xaxis.set_ticks(xlist)
ax3.xaxis.set_ticklabels(xlist.strftime("%b-%Y"), rotation = 30, ha="right", rotation_mode="anchor")

fig.tight_layout()

#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_bias_2018-2020_v5.png'),dpi=300)
#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_bias_2018-2020_v5.pdf'),dpi=300)


#%% 
"""
PART 2: Model evaluation subpanels
""" 
#data 
...
ax=axes[0]
LL_obs_dLdt['dLdt_180'].loc['1983-01-01':'2020-12-31 '].plot(ax=ax, label='observations', c='C1')
WBM_obs_dLdt['dLdt_180'].plot(ax=ax, label='WBM', c='C0')
ax.legend()
...
fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'WBM_bias_dLdt_tseries.pdf'),dpi=300)
...
LL_obs_dLdt['dLdt_180'].loc['1983-01-01':'2020-12-31 '].hist(density=True, alpha=0.9, bins=30, color='C1',label='observations')
WBM_obs_dLdt['dLdt_180'].loc['1983-01-01':'2020-12-31 '].hist(density=True, alpha=0.7, bins=30, color='C0',label='modelled')
...
fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'WBM_bias_dLdt_hist.pdf'),dpi=300)
...
plt.figure(figsize=(5.5,4))
LL_obs_dLdt['dLdt_180'].loc['1983-01-01':'2020-12-31 '].hist(density=True, alpha=0.9, bins=30, color='C0',label='observations')
WBM_obs_dLdt['dLdt_180'].loc['1983-01-01':'2020-12-31 '].hist(density=True, alpha=0.7, bins=30, color='C1',label='modelled')
#plt.xlabel('$\Delta$L/$\Delta$t for $\Delta$t=180 days [m]')
#plt.xlabel('$X$ [m]') #($\Delta$L/$\Delta$t for $\Delta$t=180 days)
plt.xlabel('$\Delta$L/$\Delta$t (m)')
plt.ylabel('density')
...
data1.plot(ax=ax, label='observations', c='C1')
data2 = WBM_obs_dLdt['dLdt_180'].groupby(WBM_obs_dLdt.index.dayofyear).mean()
data2.plot(ax=ax, label='WBM', c='C0')
# X axis 
plt.xlim(0, 365)
dates = pd.date_range(start='2014-01-01', end='2014-12-31')
ax.xaxis.set_major_locator(mdates.MonthLocator())
...
fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'WBM_bias_dLdt_climatology.pdf'),dpi=300)
...
data1 = data1.groupby(data1.index.dayofyear).mean()
data1.plot(ax=ax, label='observations')
