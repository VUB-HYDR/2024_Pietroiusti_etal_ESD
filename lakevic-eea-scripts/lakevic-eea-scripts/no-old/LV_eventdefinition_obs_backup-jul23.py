...
#plt.savefig(os.path.join(fig_path,'annualblockmax_obs_{}_{}_dt{}_v2.png'.format(startYEAR,endYEAR,dt_type)),dpi=300)


#%% Plot timeseries of blockmax for dt=180 and for different dts - not used in paper, not necessary
#=============================================================================

# 1) Plot annual blockmax timeseries for dt = 180 : alone
#=============================================================================

# select dt=180
dt_select = 180
#df_select = df_join[['year', 'dLdt_{}'.format(dt_select)]].copy()
df_select = df_join[['dLdt_{}'.format(dt_select)]].copy()
# clean up
df_select['year'] = pd.to_datetime(df_select.index, format="%Y")
df_select = df_select.set_index(['year'])

#calculate rolling mean for 5-years
df_select['rolling'] = df_select['dLdt_{}'.format(dt_select)].rolling(10, center=True).mean()

# plot step plot annual block max time-series
fig, ax = plt.subplots()
ax.step(df_select.index,df_select['dLdt_{}'.format(dt_select)], color='r', label='$\Delta$t = 180 days')
ax.plot(df_select['rolling'], color='lime', label='10-year rolling mean') 
...
plt.legend()
plt.xlabel("year")
plt.ylabel("max $\Delta$L/$\Delta$t [m/yr]")

fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'annualblockmax_dt180_v1.png'),dpi=200)




#%%
# 1.2) plot histogram distribution of variable for dt=180, not in paper
#=============================================================================
df_select['dLdt_{}'.format(dt_select)].hist()
plt.xlabel("block max $\Delta$L/$\Delta$t [m/yr]")
plt.ylabel("count")


fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'annualblockmax_dt180_hist_v1.png'),dpi=200)
...
for i in range(len(df_select)):
    ...
    ax.plot([df_select.iloc[i,3], df_select.iloc[i,4]] , #end, start
             ...
             lw=4)
    plt.axvline(x=0, ls='--', c='lightgray', lw=1)
    #plt.axvline(x=365, ls='--', c='lightgray')
    plt.xlim(-180,365)
    #plt.xlabel("\n annual block max $\Delta$L/$\Delta$t 180-day interval")
    plt.xticks(list_ticks_label, list_monthnames_labels, rotation = 45)
# plt.text(0.08, -0.18, 'previous year', transform=ax.transAxes, c='gray')
# plt.text(0.55, -0.18, 'block max year', transform=ax.transAxes, c='gray')
# if specifying portrait view
plt.text(0.08, -0.1, 'previous year', transform=ax.transAxes, c='gray')
plt.text(0.55, -0.1, 'block max year', transform=ax.transAxes, c='gray')


fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'blockmax_intervals_{}_{}_dt{}_v6.png'.format(startYEAR,endYEAR,dt_select)),dpi=300)
...
fig.tight_layout()
#plt.savefig(os.path.join(fig_path,'blockmax_intervals_{}_{}_dt{}_v2.png'.format(startYEAR,endYEAR,dt_select)),dpi=300)
#plt.savefig(os.path.join(fig_path,'blockmax_intervals_{}_{}_dt{}_v2.pdf'.format(startYEAR,endYEAR,dt_select)),dpi=300)
