...
# plot modelled 
ax.plot(WBM_obs['L_obs'], label="modelled " ) #(observational run) 
...
ax.plot(WBM_obs['L_obs'], label="modelled ", color='C1' ) #(observational run) #C0
...
ax3.set_xlabel('')
xlist =  pd.date_range(str(left), end=str(right), freq='6MS') # 2 MS for original SI figure
ax3.xaxis.set_ticks(xlist)
ax3.xaxis.set_ticklabels(xlist.strftime("%b-%Y"), rotation = 30, ha="right", rotation_mode="anchor")

fig.tight_layout()

#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_bias_2003-2010.png'),dpi=300)
#plt.savefig(os.path.join(fig_path,'LL_WBMPy_evaluation_lakelevs_bias_2003-2010.pdf'),dpi=300)
